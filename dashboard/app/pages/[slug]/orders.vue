...
definePageMeta({ layout: 'shop', middleware: ['shop-member'] })
...
async function loadOrders() {
  ...
  try {
    ...
    orders.value = Array.isArray(data)
      ? data
      : (data as PaginatedResponse<OrderInfo>).results || []
  } catch (err: unknown) {
...
function toggleExpand(orderId: number): void {
...
async function cancelOrder(orderId: number): Promise<void> {
  ...
  try {
    ...
    // Immutable: replace the order instead of mutating it in place.
