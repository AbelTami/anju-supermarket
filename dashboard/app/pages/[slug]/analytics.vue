    ...
    <div class="py-6 px-4 lg:px-6 max-w-2xl mx-auto shop-animate-in pb-24 lg:pb-6">
      ...
      <div v-else class="space-y-5">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div class="shop-card p-4 text-center">
            <p class="text-2xl font-bold tabular-nums text-(--ui-text)">
              {{ totalOrders }}
            </p>
            <p class="text-xs text-(--ui-text-muted) mt-1">
              总订单
        ...
        <!-- Member overview -->
        <div class="shop-card p-5">
          <h2 class="text-sm font-semibold text-(--ui-text) mb-3">
            会员概览
          </h2>
          <div class="grid grid-cols-2 gap-y-3 gap-x-6 text-sm">
            <div class="flex justify-between">
              <span class="text-(--ui-text-muted)">储值余额</span><span class="text-(--ui-text) font-medium tabular-nums">{{ formatPrice(balance) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-(--ui-text-muted)">总订单数</span><span class="text-(--ui-text) font-medium">{{ totalOrders }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-(--ui-text-muted)">积分</span><span class="text-(--ui-text) font-medium tabular-nums">{{ points }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-(--ui-text-muted)">累计消费</span><span class="text-(--ui-text) font-medium tabular-nums">{{ formatPrice(totalSpent) }}</span>
            </div>
          </div>
        </div>
