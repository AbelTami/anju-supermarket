...
const apiUrl = computed(() => tenantSlug.value ? `/api/tenant/${tenantSlug.value}/employees` : undefined)

const { data, status, refresh } = useFetch(apiUrl, {
  lazy: true,
  server: false,
  key: computed(() => `employees-settings-${tenantSlug.value}`),
})
const q = ref('')

const filtered = computed<EmployeeInfo[]>(() => {
  if (!data.value?.results) return []
  return data.value.results.filter(e =>
    !q.value || (e.name || e.username).includes(q.value),
  )
})

const editOpen = ref(false)
const editTarget = ref<EmployeeInfo | null>(null)
const editRole = ref('')

function openEditModal(emp: EmployeeInfo): void {
...
async function toggleEmployee(emp: EmployeeInfo): Promise<void> {
...
<template>
  <div class="space-y-6">
    ...
    <UPageCard variant="subtle" :ui="{ container: 'p-0 sm:p-0 gap-y-0', header: 'p-4 mb-0 border-b border-default' }">
      ...
      <div v-if="status === 'pending'" class="text-center py-8 text-dimmed">
        加载中...
      </div>
        ...
        <div v-for="emp in filtered" :key="emp.id" class="flex items-center justify-between p-4 hover:bg-elevated/50">
          <div>
            ...
            <div class="text-sm text-dimmed">
              {{ emp.username }} · {{ emp.role_label || '无角色' }} · {{ emp.phone }}
            </div>
          </div>
