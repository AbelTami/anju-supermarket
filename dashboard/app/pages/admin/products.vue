...
const apiUrl = computed(() => tenantSlug.value ? `/api/tenant/${tenantSlug.value}/products` : undefined)

const { data, status, refresh } = useFetch(apiUrl, {
  lazy: true,
  server: false,
  key: computed(() => `products-${tenantSlug.value}`),
})
...
const columns: TableColumn<Product>[] = [
  ...
  { accessorKey: 'is_weighted', header: '称重', cell: ({ row }) => row.original.is_weighted ? '是' : '否' },
  {
