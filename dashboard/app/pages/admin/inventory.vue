...
const { data: lowStockData } = useFetch(lowStockUrl, { lazy: true, server: false, watch: [tenantSlug] })
const lowStockCount = computed(() => (lowStockData.value as any)?.count || 0)
  ...
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar title="库存管理">
        <template #right>
          <UBadge v-if="lowStockCount > 0" color="error" variant="subtle" class="mr-2">
            {{ lowStockCount }} 个库存预警
          </UBadge>
    ...
    <template #body>
      ...
      <div class="p-4">
        ...
        <UInput v-model="search" icon="i-lucide-search" placeholder="搜索商品名/条码..." class="flex-1 mb-4" />
        <UTable
