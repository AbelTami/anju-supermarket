...
async function fetchChart() {
  ...
  try {
    ...
    data.value = ((d as any).daily || []).map((x: any) => ({ date: new Date(x.date), amount: x.amount }))
  } catch { /* keep empty */ }
}

onMounted(fetchChart)
watch([() => (auth.currentTenant.value as any)?.slug, () => props.period, () => props.range], fetchChart)
